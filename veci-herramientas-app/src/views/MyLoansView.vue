...
<template>
  <div class="min-h-screen bg-gray-100 p-8">
    <div class="mx-auto max-w-5xl">
          ...
          <div v-for="loan in loansStore.incomingRequests" :key="loan.id" class="rounded-lg bg-white p-6 shadow-sm">
            <div class="flex flex-col justify-between gap-4 md:flex-row md:items-center">
              ...
              <div class="flex flex-col items-end gap-2">
                <span :class="`px-3 py-1 rounded-full text-xs font-bold uppercase tracking-wide ${getStatusColor(loan.status)}`">
                ...
                <div v-if="loan.status === 'pending'" class="flex gap-2">
                  <button 
                    @click="handleStatusChange(loan.id, 'approved')"
                    class="rounded bg-green-500 px-3 py-1 text-sm text-white hover:bg-green-600"
                  >
                    Aprobar
                  </button>
                  <button 
                    @click="handleStatusChange(loan.id, 'rejected')"
                ...
                <div v-if="loan.status === 'approved'" class="mt-2">
                  <button 
                    @click="handleStatusChange(loan.id, 'active')"
                ...
                <div v-if="loan.status === 'active'" class="mt-2">
                  <button 
                    @click="handleStatusChange(loan.id, 'returned')"
      ...
      <!-- SECCIÓN 2: MIS SOLICITUDES (Soy el solicitante) -->
      <div v-else>
